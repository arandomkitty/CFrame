...
use MediaWiki\Skins\CFrame\Components\CFrameComponentSearchBox;
use MediaWiki\Skins\CFrame\Components\CFrameComponentVariants;
 ...
 * @internal
 */
class SkinCFrame extends SkinMustache {
	/** @var int */
	...
	private function decoratePortletData(
		...
		switch ( $key ) {
			...
			case 'data-personal':
			case 'data-user-page':
			case 'data-cframe-user-menu-overflow':
		...
		if ( $key === 'data-personal' ) {
			...
			$portletData['class'] .= ' cframe-user-menu';
		}
		...
		if ( $key === 'data-variants' ) {
			$variant = new CFrameComponentVariants(
				$this->languageConverterFactory,
				$portletData,
				$this->getTitle()->getPageLanguage(),
				$this->msg( 'cframe-language-variant-switcher-label' )
			);
	...
	/**
	 * Helper for applying CFrame menu classes to portlets
	 *
	...
	private function updatePortletClasses(
		...
		$extraClasses = [
			self::MENU_TYPE_DROPDOWN => 'cframe-menu-dropdown',
			self::MENU_TYPE_TABS => 'cframe-menu-tabs',
			self::MENU_TYPE_PORTAL => 'cframe-menu-portal portal',
	...
	public function getTemplateData(): array {
		...
		// SkinCFrame sometimes serves new CFrame as part of removing the
